init python:
    ...
    def run_stt(lang_code="id-ID"):
        ...
        python_exe = r"C:\Users\m3g3n\AppData\Local\Programs\Python\Python312\python.exe"
        ...
        try:
            ...
            raw_out, raw_err = proc.communicate(timeout=30.0)  # batasi max 15 detik
    ...
    def _normalize_text(text):
        ...
        # OPTIONAL: buang karakter non-huruf/angka (tanda baca, dsb)
        # "banjir!!!" -> "banjir"
        ...
        # OPTIONAL: kalau masih mau hilangin duplikat berurutan:
        dedup = []
        for w in tokens:
    ...
    def format_spoken(value):
        """
        Mengubah input dari STT (string atau list) menjadi string yang aman untuk renpy.notify.
        """
        if value is None:
        ...
        # Jika list atau RevertableList → join
    ...
    # -----------------------------
    # 3) Pilih opsi menu dengan suara
    # -----------------------------
    ...
    keyword_map=None
        ...
        # 1) cek kata kunci (supaya 'longsor' atau 'banjir' 1 kata pun bisa)
        if keyword_map:
            hits = []
            ...
            # Kalau tidak ada keyword yang ketemu, JANGAN pakai cosine,
            # langsung dianggap gagal.
            if len(hits) == 0:
                renpy.notify("Kamu mengatakan: " + format_spoken(spoken_tokens))
                return None

        # 2) Cosine similarity terhadap semua opsi
        best_idx = None
        best_score = None
        ...
        if best_idx is not None and best_score is not None:
            if best_score >= threshold:
                ...
                renpy.notify("Suara kurang mirip dengan pilihan yang ada.")
                return None
...
screen stt_listening_menu(options):
    ...
    zorder 100          # di atas UI lain

    window:
        style "menu_window"   # supaya mirip tampilan menu biasa
...
label pilih_bencana:
    ...
    # Kata kunci khusus, biar 1 kata pun (longsor / banjir) langsung kebaca
    $ keyword_map = {
        0: ["longsor"],
        1: ["banjir"],
    }
    ...
    niko "• Belajar tentang Longsor\n• Belajar tentang Banjir"

    jump stt_bencana
    
label stt_bencana:

    # Langsung mulai STT, tanpa click apa pun
    while True:
        ...
        niko "Silakan ucapkan:\n• Belajar tentang Longsor\n• Belajar tentang Banjir"

        jump stt_bencana   # ulangi dari awal label
...
label pilih_longsor_1:
    ...
    $ renpy.pause(1.0)
    $ renpy.notify("Klik mouse untuk mulai mendengarkan.")
    niko "• Ayo langsung laporkan ke Pak Ardi!\n• Tunggu dulu, ini cuma retakan kecil."
